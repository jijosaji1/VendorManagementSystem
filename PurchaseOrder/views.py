...
def order_details(request):
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            response = validate_data(serializer.validated_data)
            if response != None:
                return response
            serializer.save()
        ...
        print("GET request for purchase orders")
...
def order(request, purchase_order_id):
    ...
    if request.method == 'GET':
        ...
        serializer = PurchaseOrderSerializer(purchase_orders, data=request.data)
        if serializer.is_valid():
            response = validate_data(serializer.validated_data)
            if response != None:
                return response
...
def validate_data(purchase_order):
    try:
